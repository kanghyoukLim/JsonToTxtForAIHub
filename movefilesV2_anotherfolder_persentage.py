...
# 원본 폴더 내의 'txt' 파일 목록 가져오기
txt_files = [file for file in os.listdir(source_folder) if file.endswith('.txt')]

# 일정 비율로 복사할 'txt' 파일의 수 계산 (예: 전체 파일 중 30%)
percentage_to_copy = 0.3
num_files_to_copy = int(len(txt_files) * percentage_to_copy)
...
selected_pairs = selected_pairs[:min(num_files_to_copy, len(selected_pairs))]
